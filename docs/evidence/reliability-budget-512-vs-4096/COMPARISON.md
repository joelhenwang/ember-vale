...
| Beat walls | 29.2s, 52.4s, 66.5s, 35.3s | 45.9s, 112.5s, >180s timeout, 409 rejection |
...
- 4096: character, reaction, and resolver calls succeed (travel fully ok).
  ...
  eliminate it. One follow-up beat exceeded the 180s client timeout with the
  server phase stuck `scenes_assembled`; the clock never advanced, so the
  harness recorded commit ambiguity false and did not resubmit blindly. The
  next seat change (409 mid-run) and the following advance (409 open run) were
  retained as errors, and the final report still wrote with all attempts.
...
- `baseline-live-mugr0yw9.json`: 4096 arm (2/4 committed + retained timeout
  and 409s; still `complete: true` — the scenario finished, the beats did not
  all commit).
